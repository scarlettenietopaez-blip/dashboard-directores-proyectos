...
async function loadData(){
  let payload = await fetchJson(CONFIG.dataUrl);
  if(Array.isArray(payload)) payload = {updatedAt: new Date().toISOString().slice(0,10), records: payload};
...
loadData().then(() => {
  ...
  console.error(err);
  document.body.insertAdjacentHTML('afterbegin', `<div style="background:#fee2e2;color:#991b1b;padding:12px 18px;font-weight:800">No se pudo cargar la base de datos. Revisa que data/projects.json exista y esté actualizado por Power Automate.</div>`);
});
