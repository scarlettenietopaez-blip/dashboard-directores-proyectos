...
function mapCsvRow(row){
  ...
  return {
    ...
    referencia,
    proyecto: referencia,
    objetivo: col(row, "OBJETIVO GENERAL"),
    noTramite: col(row, "No TRAMITE", "NRO TRAMITE", "NRO. TRAMITE", "NÚMERO DE TRÁMITE"),
    descripcion: col(row, "DESCRIPCION GENERAL", "DESCRIPCIÓN GENERAL"),
    director: col(row, "TECNICO ENCARGADO", "TÉCNICO ENCARGADO", "DIRECTOR"),
    fechaInicial,
    ...
    observacion: col(row, "OBSERVACION", "OBSERVACIÓN"),
...
async function fetchData(url){
  ...
  if(url.toLowerCase().includes(".csv")){
    const records = csvToRows(text).map(mapCsvRow);
